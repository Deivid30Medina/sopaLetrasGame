window.addEventListener("load", function () {
  ...
  if (document.readyState === "complete") {
    // Todos los recursos y elementos de la página se han cargado
    console.log("Se cargaron todos los recursos y elementos.");
    ...
    function palabraCabeEnPosicion(sopa, palabra, fila, columna, orientacion) {
      ...
      for (let i = 0; i < palabra.length; i++) {
        if (
          !validarPosicion(sopa, filaActual, columnaActual) ||
          (sopa[filaActual][columnaActual] !== "" &&
            sopa[filaActual][columnaActual] !== palabra[i])
        ) {
          return false;
        }
    ...
    function colocarPalabrasEnSopa(palabras, filas, columnas) {
      const orientaciones = [
        ...
        [1, 0],
        [-1, 0],
        [0, -1], // horizontal, vertical, vertical invertida, horizontal invertida
        [1, 1],
        [-1, -1],
        [1, -1],
        [-1, 1], // diagonal descendente, diagonal ascendente, diagonal ascendente invertida, diagonal descendente invertida
      ];
      ...
      sessionStorage.setItem(
        "arrayPrincipal",
        JSON.stringify(palabraUbicacion)
      );
